...
class GameScene: SKScene {
    ...
    public override func keyDown(with event: NSEvent) {
        ...
        // Check for movement keys, update the delta vector, and move the player node.
    ...
    public override func keyUp(with event: NSEvent) {
        ...
        case kVK_ANSI_W, kVK_ANSI_S, kVK_ANSI_A, kVK_ANSI_D, kVK_UpArrow, kVK_DownArrow, kVK_LeftArrow, kVK_RightArrow:
            if let velocity = self.playerNode?.physicsBody?.velocity {
                if velocity != CGVector.zero {
                    self.playerNode?.physicsBody?.velocity = CGVector.zero
                }
            }

...
class AIPredeterminedTreeStrategist: NSObject, GKStrategist {
    ...
    func bestMoveForActivePlayer() -> GKGameModelUpdate? {
        ...
        guard let response = makeDecisionTree().findAction(forAnswers: answers) as? AIGamePlayerAction else {
            return defaultAction()
        }
        return AIGameDecision(by: response, with: 1)
    }
    ...
    func makeDecisionTree() -> GKDecisionTree {
        ...
        root?.createBranch(value: true, attribute: "nearExit?" as NSObjectProtocol)

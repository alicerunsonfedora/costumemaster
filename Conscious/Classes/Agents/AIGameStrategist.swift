...
class AIGameStrategist {
    ...
    var description: String {
        "AIGameStrategist(type: \(strategy.description), state: \(state))"
    }

    /// A string that provides a simple description of the strategist to a receiver.
    var simpleDescription: String {
        "AIGameStrategist(with strategy: \(strategy.description))"
    }
    ...
    init(with initialState: AIAbstractGameState) {
        state = initialState
        strategy.gameModel = initialState
        strategy.randomSource = GKARC4RandomSource()
    }

    /// Initialize a strategist.
    /// - Parameter initialState: The initial state of the game world.
    /// - Parameter budget: The maximum number of times the strategist can look forward to.
    init(with initialState: AIAbstractGameState, budget _: Int) {
        state = initialState
        strategy.gameModel = initialState
        strategy.randomSource = GKARC4RandomSource()
    }
    ...
    func nextAction() -> AIGameDecision {
        guard let action = strategy.bestMoveForActivePlayer() as? AIGameDecision else {
